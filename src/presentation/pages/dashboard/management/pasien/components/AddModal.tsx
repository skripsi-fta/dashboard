import {
    managementPatientCreateValidation,
    type ManagementPatientCreate,
...
const AddModal = ({ refetch }: AddModalProps) => {
    const { control, handleSubmit } = useForm<ManagementPatientCreate>({
        defaultValues: {
            address: '',
            dateOfBirth: '',
            idNumber: '',
        ...
        mode: 'onChange',
        resolver: zodResolver(managementPatientCreateValidation)
    });
    ...
    const { mutate: create, isLoading } = useMutation({
        mutationFn: (data: ManagementPatient.Request.Create) =>
            api.create(data),
        onSuccess: () => {
            toast.success('Sukses membuat pasien');
            closeModal();
            refetch();
        },
        onError: (res: AxiosError<{ message: string }>) => {
            toast.error(res.response?.data.message ?? 'Membuat pasien error');
        }
    });
