...
const CashierReportComponent = () => {
    ...
    const { data: dataSummary, isLoading: isLoadingSummary } = useQuery({
        queryKey: ['pharmacy-summary', filterValues, 'cashier'],
        queryFn: () => managementReportAPI.getCashierSummary(filterValues),
        onError: () => {
    ...
    const { data: dataList, isLoading: isLoadingList } = useQuery({
        queryKey: ['pharmacy-summary', filterValues, pagination, 'cashier'],
        queryFn: () =>
            managementReportAPI.getCashierData({
                ...filterValues,
                pageSize: pagination.pageSize,
                pageNumber: pagination.pageIndex + 1
            }),
        onError: () => {
            toast.error('Get cashier report error');
        }
    });
    ...
    const columns: ColumnDef<ManagementReport.Response.Data>[] = [
        ...
        },
        {
            ...
            size: 150,
            header: 'Waktu Mulai Kasir'
        },
        {
            accessorKey: 'endTime',
            size: 150,
            header: 'Waktu Selesai Kasir'
        }
    ];
                                ...
                                <div className='flex flex-row items-center gap-4'>
                                    <div className='flex h-full flex-1 flex-col gap-3 rounded-[16px] bg-white p-6'>
                                        <p className='text-xl font-semibold text-[#343C6A] md:text-2xl'>
                                            Rata-rata Waktu Menunggu Kasir
